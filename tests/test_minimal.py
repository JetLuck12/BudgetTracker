"""
Минимальные тесты для Budget Tracker - только основные функции
"""
import pytest
import os
import tempfile
...
class TestSummary:
    ...
    def test_get_financial_summary_basic(self):
        """Тест получения финансовой сводки с простыми данными"""
        # Создаем простой список транзакций
        transactions = [
        ...
        assert summary["expense"] == 1500.0  # 1000 + 500
        assert summary["balance"] == 500.0
        assert summary["count"] == 3
        assert summary["avg_check"] == 2000.0 / 3  # 2000 / 3
    
    def test_get_summary_by_category_all(self):
        ...
        summary = Summary.get_summary_by_category(transactions, tran_type.All)
        
        assert summary["Зарплата"] == 2000.0
        assert summary["Продукты"] == -1000.0
        assert summary["Транспорт"] == -500.0
    
    def test_get_summary_by_category_income_only(self):
        ...
        summary = Summary.get_summary_by_category(transactions, tran_type.Income)
        
        assert summary["Зарплата"] == 2000.0
        assert summary["Продукты"] == 0.0
        assert summary["Транспорт"] == 0.0
    
    def test_get_summary_by_category_expense_only(self):
        ...
        summary = Summary.get_summary_by_category(transactions, tran_type.Outcome)
